...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const Sidebar = ({ isMobile }: { isMobile?: boolean }) => {
    ...
    return (
      <div className={`flex flex-col h-full ${showFull ? 'w-60' : 'w-16'} transition-all duration-300`}>
        {/* Logo */}
        <div className={`flex items-center gap-3 p-5 border-b border-border/50 ${showFull ? '' : 'justify-center'}`}>
          ...
          {showFull && <div><div className="font-display font-bold text-sm gradient-text"></div><div className="text-xs text-muted-foreground">Admin Panel</div></div>}
        </div>
          ...
          {NAV.map(({ label, href, icon: Icon }) => {
            ...
            return (
              <Link key={href} href={href} onClick={() => setMobileOpen(false)}
                ...
                {showFull && <span className="flex-1">{label}</span>}
                {showFull && active && <ChevronRight className="w-3 h-3 opacity-50" />}
              </Link>
            )
          })}
        </nav>
        ...
        <div className={`p-3 border-t border-border/50`}>
          ...
          <button onClick={handleLogout}
            className={`w-full flex items-center gap-3 px-3 py-2 rounded-xl text-sm text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-all ${showFull ? '' : 'justify-center'}`}>
